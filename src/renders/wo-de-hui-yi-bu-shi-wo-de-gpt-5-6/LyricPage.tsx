...
import { FONT_BODY, FONT_IMPACT, FONT_LATIN, FONT_MEMORY } from "./fonts";
...
const transformFor = (motion: TextMotion, enter: number, index: number) => {
  const direction = index % 2 === 0 ? -1 : 1;
  switch (motion) {
    case "expose":
      return `translateX(${(1 - enter) * -18}px) scaleX(${0.72 + enter * 0.28})`;
    case "settle":
      return `translateY(${(1 - enter) * -17}px) rotate(${(1 - enter) * direction * 1.8}deg)`;
    case "unlock":
      return `translateY(${(1 - enter) * 19}px) skewX(${(1 - enter) * -5}deg)`;
    case "breathe":
      return `translateY(${(1 - enter) * 10}px) scale(${0.94 + enter * 0.06})`;
    case "register":
      return `translate(${(1 - enter) * -11}px, ${(1 - enter) * 7}px) rotate(${(1 - enter) * -1.2}deg)`;
    case "slide":
    default:
  ...
  return (
    <div
      style={{
        ...
        opacity: enter,
        transform: `translateX(${(1 - enter) * -18}px) scaleX(${0.985 + enter * 0.015})`,
