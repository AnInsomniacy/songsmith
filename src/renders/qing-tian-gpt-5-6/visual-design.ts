...
export const getPhraseFont = (phrase: string, motion: TypeMotion) => {
  if (motion === "bloom" || motion === "swing")
    return '"Kaiti SC", "STKaiti", "Songti SC", serif';
  if (motion === "rain") return '"Noto Serif CJK SC", "Songti SC", serif';
  if (motion === "choice" || motion === "farewell")
    return '"Hiragino Sans GB", "Heiti SC", sans-serif';
  if (motion === "wind") return '"Heiti SC", "Hiragino Sans GB", sans-serif';
  if (motion === "sun") return '"Noto Serif CJK SC", "Songti SC", serif';
  if (/故事|从前|记忆/u.test(phrase))
    return '"Songti SC", "Noto Serif CJK SC", serif';
  return '"Kaiti SC", "STKaiti", "Songti SC", serif';
};
