...
import type { CharacterTiming, TimedLine } from "./types";
...
const STORY_FONT = '"Noto Serif CJK SC", "Songti SC", serif';
const DISPLAY_FONT = '"Hiragino Sans GB", "Heiti SC", sans-serif';
const HAND_FONT = '"Kaiti SC", "STKaiti", "Songti SC", serif';
const NOTE_FONT =
  '"DIN Condensed", "Avenir Next Condensed", "PingFang SC", sans-serif';
...
const buildRevealFrames = (timings: CharacterTiming[], fps: number) => {
  let previous = -1;
  return timings.map((timing) => {
    const sourceFrame = Math.round((timing.startMs / 1000) * fps);
    const scheduled = Math.max(sourceFrame, previous + 1);
    previous = scheduled;
    return scheduled;
  });
};
...
const enterTransform = (
  ...
  const settle = Easing.bezier(0.16, 1, 0.3, 1)(progress);
  if (motion === "bloom")
    return `translateY(${(1 - settle) * 34}px) scale(${0.62 + settle * 0.38}) rotate(${(1 - settle) * (index % 2 === 0 ? -8 : 8)}deg)`;
  if (motion === "swing")
    return `translateY(${(1 - settle) * (index % 2 === 0 ? -34 : 34)}px) rotate(${(1 - settle) * (index % 2 === 0 ? -11 : 11)}deg)`;
  if (motion === "rain")
    return `translateY(${(1 - settle) * -42}px) scaleY(${1.42 - settle * 0.42})`;
  if (motion === "choice")
    return `translateY(${(1 - settle) * (index % 2 === 0 ? -42 : 42)}px) scale(${0.78 + settle * 0.22})`;
  if (motion === "wind")
    return `scaleX(${0.08 + settle * 0.92}) skewX(${(1 - settle) * -13}deg)`;
  if (motion === "sun")
    return `translateY(${(1 - settle) * 28}px) scale(${0.72 + settle * 0.28})`;
  if (motion === "farewell")
    return `translateY(${(1 - settle) * 50}px) rotate(${(1 - settle) * -7}deg) scale(${0.7 + settle * 0.3})`;
  return `translateY(${(1 - settle) * 24}px) scale(${0.82 + settle * 0.18})`;
};
  ...
  return (
    <span
      style={{
        ...
        display: "inline-flex",
        alignItems: "center",
        width: cellWidth * characters.length,
        ...
        return (
          <span
            ...
            style={{
              ...
              transform: `translateY(-50%) ${enterTransform(motion, progress, index)}`,
              transformOrigin:
...
const NoteLine: React.FC<{
  ...
  const matches = [...line.text.matchAll(/Re|So|Si|Do|La/gu)];
  let previous = -1;
  const notes = matches.map((match) => {
    const index = match.index ?? 0;
    const raw = Math.round(
      ((line.characters[index]?.startMs ?? line.startMs) / 1000) * fps,
    );
    const revealFrame = Math.max(raw, previous + 1);
    previous = revealFrame;
    return { text: match[0], revealFrame };
  });
  return (
    <div
      style={{
          ...
          return (
            <div
              ...
              style={{
                ...
                fontSize: 42,
                fontWeight: 900,
                transform: `translateY(${(1 - enter) * 34}px) scale(${0.68 + enter * 0.32}) rotate(${(1 - enter) * (index % 2 ? 5 : -5)}deg)`,
...
const LyricLine: React.FC<{
  ...
  const revealFrames = buildRevealFrames(line.characters, fps);
  ...
  return (
    ...
    >
      {phrases.map((phrase, phraseIndex) => {
        ...
        const timings = line.characters.slice(offset, offset + count);
...
export const QingTianVideo: React.FC<QingTianVideoProps> = ({ audioFile }) => {
